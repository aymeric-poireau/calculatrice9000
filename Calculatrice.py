...
class Calculator():
    ...
    def Racine(self): # Racine carrée
        nb = self.text.split("V")
        self.phase1 = float(nb[0])
        self.phase2 = float(1/2)
        self.final = self.phase1 ** self.phase2
        self.entry.set(str(self.final))
        self.init()     

    def Pour(self): # Pourcentage
        nb = self.text.split("%")
        self.phase1 = float(nb[0])
        self.phase2 = float(nb[1])
        self.final = (self.phase1 * 100) / self.phase2
        self.entry.set(str(self.final))
        self.init()
...
def ButtonA (): # Actionnerle bouton A
    calculatrice.text += "+"
    calculatrice.entry.set(calculatrice.text)
...
def ButtonR (): # Actionnerle bouton R
    calculatrice.text += "V"
    calculatrice.entry.set(calculatrice.text)
...
fen.geometry("226x320") # Définition de la fenêtre
fen.title("Calculatrice 9000") # Titre de la calculatrice
...
BR = Button(fen, text="V", command=ButtonR, width=4, height=2, bg="gold", fg="white", relief=GROOVE).place(x=150, y=200)
BP = Button(fen, text="%", command=ButtonP, width=4, height=2, bg="gold", fg="white", relief=GROOVE).place(x=150, y=240)

BA = Button(fen, text="+", command=ButtonA, width=4, height=2, bg="gold", fg="black", relief=GROOVE).place(x=150, y=40) # Boutton + (addition)
...
BE = Button(fen, text="=", command=ButtonE, width=4, height=1, bg="blue", fg="white", relief=RIDGE).place(x=150, y=290) # Button = (égale)
